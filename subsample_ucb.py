...
def subsample_ucb(total_time_slot, arm_num, seed=10):
    ...
    for i in range(total_time_slot - subsample_arm_num):
        # calculate ucb term
        reward_hat = [ave_reward[j]+np.sqrt(2*np.log(1+i)/roll_time[j]) for j in range(subsample_arm_num)]
...
if __name__ == '__main__':
    a_reward, r_time, sum_reward, cumulative_regret = subsample_ucb(total_time_slot=500, arm_num=100)
